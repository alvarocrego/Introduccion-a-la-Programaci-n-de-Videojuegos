...
#define CBS_WIDTH	1.0F
#define CBS_HEIGHT	0.6F
...
UGKS_String CBS_NameState[CBS_MAXSTATE] =
{
	"DEFAULT",
	"BORN",
	"LIVING",
...
UGKS_String CBS_NameTransition[CBS_MAXTRANSITION] =
{
	"DEFAULT",
	"BORNING",
	"GETTING_ALIVE",
...
void CBoss::Init()
{
	Health = MaxHealth = CBS_MAX_HEALTH;
	Hit_duration = CBS_MAX_HIT_DURATION;
	SubType = CBS_BOSS_;
	Type = CHARS_BOSS;
...
#ifdef CHAR_USE_AABB
	UpdateAABB(CBS_WIDTH, CBS_HEIGHT, 0.0f);
#endif
...
//
//void CBoss::AI_Init(SIAI_AI_TYPE AIType) 
//{
//	switch(AIType)
//	{
//		case SIAI_BOSS_DEFAULT:
//			AI = AIManager.GetAI(SIAI_BOSS_DEFAULT);  ///<Asks the AIManager for the 'FSM' corresponding to the Boss appropiately initialized.
//		break;
//		default:;
//	}//Behaviour switch
//}
...
void CBoss::AI_Healthing()
{
	if (Health < MaxHealth)
		Health += Timer[CBS_RND_PERIOD].GetAlarmPeriodms()*0.0002f;
...
void CBoss::AI_Move()
{
	...
	MoveRelTo(Speed.v[XDIM] * Timer[CBS_UPD_PERIOD].GetAlarmPeriodms(), 0.0f, 0.0f);
	if (Position.v[XDIM] < -CBS_MAX_X_SHIFT)			// Change movement direction
	{
		//Infinite acceleration
		if (0>Speed.v[XDIM]) Speed.v[XDIM] = -Speed.v[XDIM];
	}
	else if (Position.v[XDIM] > CBS_MAX_X_SHIFT)
		//Infinite acceleration
		if (0<Speed.v[XDIM]) Speed.v[XDIM] = -Speed.v[XDIM];
...
void CBoss::AI_Die(void)
{
	///Changing internal attributes
	SetDefault();
	Explosion.Health = float((rand() % 100) + CBS_MAX_HEALTH);
...
bool CBoss::OutEvent(CBS_BOSS_TRANSITIONS EventName) { return AI->outEvent(CBS_NameTransition[EventName], NULL, this); }
...
void CBoss::Collided(CCharacter *CollidedChar)
{
	...
	if (CHARS_COLLISION_TABLE[CHARS_BOSS][CollidedChar->Type])
		...
		case CHARS_SHOOT:
			...
			if (CSH_PLAYER2D == typeAux || CSH_PLAYER3D == typeAux || CSH_PLAYER3D_CHEVRON == typeAux || CSH_AUX_LASER == typeAux)
			{
				...
				if (Health <= 0)
				{
					...
					AI_Die();
					Explosion.Init(this, Timer[CBS_UPD_PERIOD].GetAlarmPeriodms());
					Explosion.Activate();
...
void CBoss::Render()
{
	...
	case CHAR_3D:
		...
		// Boss touché
		if (Hit_duration < CBS_MAX_HIT_DURATION)
		{
			Hit_duration -= 10.0 * Timer[CBS_UPD_PERIOD].GetAlarmPeriodms();
			if (Hit_duration <= 0) Hit_duration = CBS_MAX_HIT_DURATION;
		}
		// Boss normal
		Mesh->modelo.Draw();
...
void CBoss::Update(void)
{
	...
	if (Alive())
	{
		...
		if (Timer[CBS_UPD_PERIOD].IsSounding())
		{

#ifndef XM_CONTINUOUS_WITH_SIMULATE_TIME
			Timer[CBS_UPD_PERIOD]->ResetAlarm();

			//Next execution time calculation. (TicksToUpdateBoss)
			double auxX = abs(CharAABB.AABB[CHAR_BCBSIZE][XDIM].Value / Speed.v[XDIM]);
			double auxY = abs(CharAABB.AABB[CHAR_BCBSIZE][YDIM].Value / Speed.v[YDIM]);
			//double auxZ= abs(CharAABB.AABB[CHAR_BCBSIZE][ZDIM].Value/Speed.v[ZDIM]);	
			msUpdSShip = UGKALG_Min(auxX, auxY);
			if (msUpdSShip > SIGLBD_MAX_UPDATETIME) msUpdSShip = SIGLBD_MAX_UPDATETIME;
			Timer[CBS_UPD_PERIOD].SetAlarm(Timer[CBS_UPD_PERIOD].ms2Ticks(msUpdSShip));
			...
			if ((floor((rand() % 100000 / (1 + Navy->ShootsFrequency)) / msUpdSShip)) == 1) ///Has the Supply ship to fire?
			{
				...
				if (Navy->WithShoots)
					ShootsManager->NewShoot(CSH_BOSS, P, S);

			}

			//Move the supply ship
			OutEvent(CBS_MOVING);	//v 2->2
#else
			bool SynWithRealTime = false;
			if (Timer[CBS_UPD_PERIOD].IsSounding()) Timer[CBS_UPD_PERIOD].AdvanceOneAlarmPeriod();
...
void CBoss::DiscreteUpdate(void)
{
	...
	if (Alive() && !EndByTime && !EndByFrame)
	{
		...
		//Shooting calculation
		if ((floor((rand() % 100000 / (1 + Navy->ShootsFrequency)) / Timer[CBS_UPD_PERIOD].GetAlarmPeriodms())) == 1) ///Has the Supply ship to fire?
		{
			Vector P, S;
		...
		msgUpd->Type = UMSG_MSG_NAVY;
		msgUpd->SubType = UMSG_UPDSSHIPS;
		SendSelfMsg(msgUpd, Timer[CBS_UPD_PERIOD].ms2Ticks(msUpdSShip));
...
{
	// Boss dead
	Boss->Explosion.Init(Boss, Boss->Timer[CBS_UPD_PERIOD].GetAlarmPeriodms());
	Boss->AI_Die();

	RTDESK_CMsg *Msg = Boss->GetMsgToFill(UMSG_MSG_BASIC_TYPE);
	Boss->SendMsg(Msg, Boss->Directory[CHARS_GAME_REF], RTDESKT_INMEDIATELY);
